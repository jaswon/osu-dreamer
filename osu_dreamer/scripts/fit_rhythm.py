...
default_config_path = './osu_dreamer/rhythm_model/model.yml'
...
def fit_rhythm(config: str, ckpt_path: Optional[str]):
    """begin a training run."""

    parser = LightningArgumentParser()
    parser.add_argument('seed_everything', type=Union[int, bool])
    parser.add_lightning_class_args(RhythmModel, 'model')
    parser.add_lightning_class_args(Data, 'data')
    parser.add_lightning_class_args(Trainer, 'trainer')

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        cli = LightningCLI(RhythmModel, Data, args=parser.parse_path(config), run=False)
