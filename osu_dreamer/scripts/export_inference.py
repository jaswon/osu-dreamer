...
@click.option('--style-prior-ckpt-path', type=click.Path(exists= True, dir_okay=False), default='style_prior.ckpt', help='path to the style prior checkpoint')
@click.option(          '--output-path', type=click.Path(exists=False, dir_okay=False), default='inference.pt', help='artifact output path')
def export_inference(latent_ckpt_path: str, denoiser_ckpt_path: str, style_prior_ckpt_path: str, output_path: str):
    """export an inference model artifact from training checkpoints"""

    save_inference(latent_ckpt_path, denoiser_ckpt_path, style_prior_ckpt_path, output_path)