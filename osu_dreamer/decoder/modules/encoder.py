...
class tokenMixer(nn.Module):
    def __init__(self, dim: int, expand: int):
        super().__init__()
        self.fore = MinGRU(dim, dim*expand)
        self.back = MinGRU(dim, dim*expand)
        self.mix = MP.Linear(dim*expand*2, dim)

    def forward(self, x: Float[Tensor, "B L D"]) -> Float[Tensor, "B L D"]:
        x = x.transpose(1,2)
        f = self.fore(x)
        b = self.back(x.flip(-1)).flip(-1)
        return self.mix(th.cat([f,b], dim=1).transpose(1,2))
