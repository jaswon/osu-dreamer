...
class PowerSphericalVariationalBottleneck(nn.Module):
    def __init__(self, h_dim: int, dim: int):
        super().__init__()
        self.loc = MP.Conv1d(h_dim, dim, 1)
        self.log_scale = nn.Conv1d(h_dim, 1, 1)
    ...
    def forward(
        ...
        z = rearrange(posterior.rsample(), '(b l) d -> b d l', b=z.size(0))
        
        prior = HypersphericalUniform(loc_flat.size(1), device=z.device, dtype=z.dtype) # type: ignore
